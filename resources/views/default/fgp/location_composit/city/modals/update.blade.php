<div class="modal-dialog modal-custom-medium-width" role="document">
    <div class="modal-content mp0">
                                    ...
                                    <div class="form-group  row mp0">
                                        <div class="col-lg-3 field">
                                            ...
                                            <select name="district_id" id="district_id">
                                                @if($city->district)
                                                <option value="{{$city->district_id}}">{{($city->district->district_name)?:"Select"}}</option>
                                                    @endif;
                                            </select>
                                        </div>
                                        ...
                                        <div class="col-lg-2 col-md-5 field">
                                            <label for="zip_code" class="{{ is_valid('zip_code', $validations) }}">City Code</label>
                                            <input type="number" 
                                                name="zip_code" 
                                                  value="{{($city->zip_code)?:''}}"
                                                id="zip_code" 
        ...
        <div class="modal-footer d-ib">
            ...
            <button type="button" class="float-right btn btn-info m-btn--pill" id="btnUpdateCity" data-id="{{$city->id}}" data-target="newCityCreate">
                Update
            </button>
        </div>
    </div>
...
$(function(){
    ...
    /*-------- submit form ----------*/
    $(document).off('click','#btnUpdateCity').on('click','#btnUpdateCity', function (e) {
        var form = $(this).attr('data-target');
        let city = $(this).attr('data-id');
        var request = {
            url: '/location/city/update/'+city,
            method: 'post',
            form: form
        };
        addFormLoader();
