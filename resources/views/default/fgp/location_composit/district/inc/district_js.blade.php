...
var vDatatable = $('#district_data_table').mDatatable({
    // datasource definition
    data: {
        type: 'remote',
        source: {
            read: {
                url: '/location/all/district',
                method:'GET'
            },
        },
        pageSize: 20,
    ...
    columns: [
       {
        ...
        },
        {
            ...
            title: 'Action',
            template: function(row){
                return `<button class="m-portlet__nav-link btn m-btn m-btn--hover-primary m-btn--icon m-btn--icon-only m-btn--pill" data-modal-route="/location/district/edit/${row.id}">\
                        <i class="la la-edit"></i></button>
                        
                        <button class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" data-modal-route="/location/district/delete/${row.id}">\
