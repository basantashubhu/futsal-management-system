    ...
    <div class="modal-content">
        ...
        <div class="modal-body has-divider">
            <form id="holiday" class="m-form m-form--label-align-right m-form--group-seperator-dashed">
                ...
                <div class="form-group np-pd-left np-pd-bottom m-form__group row">
                    <div class="col-sm-6">
                        <label for="supervisorInput" class="col-form-label">Supervisors</label>
                        <select name="sites[]" id="supervisorInput" class="form-control" title="All" multiple>
                            @foreach ($supervisors as $supervisor)
                            <option value="{{ $supervisor->member->fullname() }}">{{ ucwords($supervisor->member->fullname()) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
...
$(function() {
    ...
    $('#generateReport').off('click').on('click', function () {
        ...
        var request = {
            url: '/fgp_generate/ApproveVolunteerReport/'+format,
            method: 'post',
            data : {
                filtered: 'false',
                query: {
                    ...
                    example3: {
                        name: 'supervisors',
                        value: $('#supervisorInput').val().join(',')
                    },
                    example4: {
                        name: 'site_name',
                        value: $('#ReportSites').val().join('|')
                    }
                },
                saveLog: '1'
            }
